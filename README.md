...
本版本针对题目生成页面和 Markdown 展示做了专项修复。

### 20.1 修复题目生成结果不符合题型的问题

旧版本主要依赖 Prompt 约束，小模型可能出现以下情况：

1. 选择题只生成 A、B、C、D 四段内容，没有明确题干；
2. 切换题型、难度、数量后，模型没有严格按参数输出；
3. 模型把资料内容直接整理成列表，而不是生成规范练习题；
4. 生成内容被包在 Markdown 代码块中，前端显示效果很差。
...
清洗 Markdown 外层代码围栏
        ↓
检查题型、数量、题干、答案、解析是否完整
        ↓
合格：直接返回模型结果
不合格：自动生成格式稳定的兜底题目
        ↓
保存到 generation_records 表
```
...
题目生成页现在默认选择资料管理中最新上传的一份资料。即使前端没有传 `document_id`，后端也会自动读取最新资料作为默认上下文。
...
### 20.3 修复 Markdown 显示效果问题

v5 对 `MarkdownView` 做了调整：

1. 自动移除包裹整段内容的 ` ```markdown ` 代码围栏；
2. 代码块改成浅色样式，避免生成结果出现大块黑底；
3. A、B、C、D 选项会被识别为选项块；
4. 题目标题会以蓝色浅背景块展示，阅读更清楚。

相关代码：

```text
frontend/src/components/MarkdownView.jsx
frontend/src/components/ResultBox.jsx
```

### 20.4 设计重点

这次修复体现了 AI 应用中的“工程化兜底”设计：

> Prompt 只能提高模型按格式输出的概率，但不能保证 100% 稳定。实际项目中，后端必须对模型输出做校验、清洗和兜底，否则前端展示就容易失控。

这也是 AI 应用和普通 API 应用不同的地方：

```text
普通接口返回结构稳定
AI 模型返回内容不完全稳定
所以 AI 应用必须增加输出校验和格式整理
```
...
1. 后端新增 `GET /api/models` 接口，用于读取 Ollama 本机已安装模型；
2. 智能问答页新增模型下拉框，选择后会作用于本次问答；
3. 题目生成与知识总结页新增同样的模型下拉框；
4. 页面仍保留“手动输入”模式，方便使用自定义模型名称；
5. 若选择未安装模型，需要先在命令行执行：

```bash
ollama pull qwen2.5:1.5b
```

设计说明：模型切换并不是修改代码，而是前端把选择的模型名传给 FastAPI，FastAPI 再把模型名传给 Ollama。


## v8 环境兼容与编码修复

本版本针对 Windows / Python 3.13 环境做了兼容性修复：

1. 更新后端依赖版本范围，降低 `pydantic-core` 本地编译失败概率；
2. 将 `uvicorn[standard]` 调整为 `uvicorn`，减少 httptools、watchfiles 等原生扩展依赖；
3. 新增 `.editorconfig`、`.gitattributes`、`.vscode/settings.json`，统一 UTF-8 编码；
4. 后端启动入口增加标准输出 UTF-8 配置；
5. README 增加 pydantic-core / maturin / cargo 报错处理说明；
6. README 增加中文乱码处理说明。

## v9 命令行启动方式整理

本版本移除 Windows 批处理文件，后端安装与启动统一通过 README 中的命令行步骤完成。

调整内容：

1. 删除 Windows 批处理文件；
2. 后端启动文档统一使用 PowerShell、macOS/Linux 命令；
3. 检查脚本不再要求批处理文件存在；
4. 保留 UTF-8 编码配置和 Python 3.13 依赖兼容处理。
